...
## Current Stage
As of 2026-03-07, the repository has moved from a scenario demo to a backend-managed SSH desktop app with a real transport path and direct terminal typing inside a managed terminal surface. The current stabilization pass is focused on live connect-state projection, terminal transcript preservation, lifecycle visibility, managed-command operator control, reconnect/disconnect operator affordances, failure-context quality during interactive SSH sessions, and shrinking the desktop shell toward a denser terminal-first layout.
...
- Collapsed saved-host editing to a single compact editor popover: the left rail now remains the only host list, while Manage and right-click Edit both open direct inline editing for the currently targeted host.
- Made the saved-host editor draggable and proportionally resizable so operators can enlarge or reposition it instead of being blocked by a fixed overlay size.
- Repacked the saved-host editor into a denser desktop form by removing the unused region field, compressing header chrome, and regrouping address, port, credentials, and key material onto fewer rows.
- Switched the saved-host editor to auth-driven detail boxes: password auth now reveals a compact password panel, private-key auth reveals the real key-path panel, and tags/fingerprint moved into a collapsible identity section.
- Trimmed host-editor vertical spacing further by removing the identity heading, flattening row gaps, and tightening the password detail card.
- Flattened host-editor secondary sections by removing the remaining tags/fingerprint and auth-card chrome, leaving inline fields and inline auth controls instead of boxed subpanels.
- Fixed the editor grid to pin rows to the top of the floating window so unused vertical space no longer expands the gaps between form rows.
- Reduced per-field vertical occupancy again by forcing grouped editor rows to min-content sizing and zero row-gap in the host editor grid.
- Replaced the editor form grid with a flex-stack layout so row spacing is determined explicitly by sequential blocks rather than implicit CSS grid track sizing.
- Reduced editor dead space further by making the default floating editor height auth-sensitive: smaller for agent mode, taller only when password/private-key details are present.
- Removed the editor close button and proportional resize affordance again, leaving a drag-only floating editor with a smaller default height to keep more of the terminal visible.
- Fully removed host-editor resize behavior from both the React component and CSS so the editor can no longer be resized accidentally by lingering chrome or stale handlers.
- Restored drag interaction across the editor surface and relaxed drag bounds so the floating editor can be moved outside the main content region without requiring a tiny title strip.
...
- Hardened the Inspect tab switcher into a fixed three-column compact bar so Timeline, Diagnosis, and Artifacts remain horizontal pills instead of collapsing into tall vertical buttons under drawer density tweaks.
- Locked the Inspect drawer header into a fixed title-and-close grid and clamped the interrupt/failure summary copy so Ctrl+C and other long summary states no longer distort the upper drawer layout.
- Corrected the Inspect drawer grid row model so the tab switcher now occupies its own auto-sized row instead of inheriting the scrollable content track, preventing the tab bar from stretching into a tall panel during interrupt and failure states.
- Tightened SSH metadata probe parsing so Talon only accepts shell and cwd markers when they begin a sanitized output line, preventing echoed probe commands like `sed 's#^#__TALON_META_CWD__#'` from corrupting session metadata with values such as `#'`.
- Extended managed-shell echo suppression to cover the internal post-connect metadata probe commands, so the terminal now keeps the final `Remote shell ready in ...` notice without showing the raw `printf` and `pwd | sed` probe lines.
- Hardened desktop startup loading so the workspace snapshot gets short retry windows and registry/agent-settings fetches fail independently, preventing transient backend startup races from dropping the app into `No workspace state available` on launch.
- Replaced the misleading startup hard-failure screen for empty registries with a real terminal-first welcome workspace: when no saved host or live session is available, Talon now offers direct `New Host` entry instead of claiming the workspace snapshot is unusable.
- Tightened managed-shell interrupt UX so the frontend no longer injects duplicate local `^C` / `Interrupt sent to remote shell...` transcript lines and now gates repeated Ctrl+C sends while a managed interrupt is already in flight.
- Reworked the Diagnosis narrative area into a denser inspect-drawer log feed with compact source/tone metadata, shorter rows, and clamped body copy so operator notes read more like a terminal-side incident log than stacked cards.
