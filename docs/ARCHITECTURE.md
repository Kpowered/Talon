...
- Host config management:
  ...
  - command completion currently applies a simple health rule: `exit 0` restores `host.observed.status` to `healthy`, while non-zero exits downgrade it to `warning`
