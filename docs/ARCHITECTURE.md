...
- host creation remains card-based, but saved-host management is now rendered as a compact floating editor popover beside the left rail so the rail itself remains the single host list while edit/delete actions stay available without obscuring the terminal
