...
use crate::session_store::{
    self, DiagnosisMessage, DiagnosisResponse, FailureContext, Host, Session, SuggestedAction,
    TalonWorkspaceState, TerminalSnapshot, TimelineEvent,
};
...
fn complete_active_command(registry: &mut SessionRegistry, session_id: &str, command_id: &str, exit_code: i32, cwd: &str) {
    ...
    if exit_code != 0 {
        let severity = if exit_code == 1 { "warning" } else { "critical" };
        let (host_id, shell) = registry
            .managed_sessions
            .iter()
            .find(|session| session.id == session_id)
            .map(|session| (session.host_id.clone(), session.shell.clone()))
            .unwrap_or_else(|| ("unknown-host".into(), "sh".into()));
        ...
        registry.latest_failures.insert(
            ...
            FailureContext {
                ...
                summary: format!(
                    ...
                    command.command, exit_code, host_id
                ),
                ...
                captured_at: completed_at.clone(),
            },
        );
        push_event(
            registry,
...
fn build_diagnosis_from_failure(failure: &FailureContext) -> DiagnosisResponse {
    DiagnosisResponse {
...
fn timeline_for_session(registry: &SessionRegistry, session_id: &str, failure: Option<&FailureContext>) -> Vec<TimelineEvent> {
    let mut timeline: Vec<TimelineEvent> = registry
        .command_history
        .iter()
...
pub fn workspace_state() -> TalonWorkspaceState {
    ...
    if let Some(active) = state.sessions.iter().find(|session| session.id == state.active_session_id) {
        ...
        if let Some(failure) = registry.latest_failures.get(&active.id) {
            ...
            state.latest_diagnosis = build_diagnosis_from_failure(failure);
            state.timeline = timeline_for_session(&registry, &active.id, Some(failure));
        } else {
            state.timeline = timeline_for_session(&registry, &active.id, None);
        }
    }
